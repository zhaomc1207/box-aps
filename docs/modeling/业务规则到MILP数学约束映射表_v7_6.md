# BOX APS业务规则到 MILP 数学约束映射表 v7.6
...
本版 **`v7.6`** 给出 BOX APS **主 MILP + 前置数据准备 + 班内细排 + 解后校核** 的完整数学建模说明。本文档直接面向实现，目标是把输入表、业务语义、可行域、目标函数、二阶段排序与审计口径一次性定义清楚，使不同实现者在不依赖口头解释的情况下也能构造语义一致的模型。

相较 **`v7.5`**，本版仅作增量微调：不扩大主 MILP 为订单级连续时间模型，而是在班内细排与输出回写阶段补充轻量时间轴，用于生成 `schedule_start_time` / `schedule_end_time`，并以低成本方式处理分钟级 FAI `lead_time`。
...
6. **FAI、换线、材料排序、lot 连续性**在主模型中只保留必要的聚合表达；分钟级 FAI 等待默认由二阶段轻量时间轴与输出回写处理，不在主问题中展开订单级开始时间变量。
...
3. **班内细排层**：对主 MILP 已分配的线体–槽位产量做顺序化，体现 FIX 顺序、FAI 组内先后、机型聚合、lot 连续与材料排序键，并生成轻量执行时间轴。
4. **解后校核层**：审计 FAI 日历口径、同槽拆线、实际换线摘要、班内时间越界、锁定一致性与交付统计。
...
对每个线体–槽位 $(l,t)$，可定义分钟级 FAI 容量预留：

- $B_{lt}^{\mathrm{fai}}\ge 0$：为该槽内可能发生的 FAI 等待预留的机时，单位为小时。

$B_{lt}^{\mathrm{fai}}$ 只表示容量侧的微小占用，不表示 FAI 先后已经在主模型中被满足。
...
当 `lead_time` 为分钟级、且不启用主模型 FAI Big-M 时，可在候选构造后为线体–槽位预先计算 $B_{lt}^{\mathrm{fai}}$，作为二阶段 FAI 等待的容量余量。

推荐的轻量口径为：对每个 FAI 组 $g$，若首件需求与至少一个余量需求在同一 $(l,t)$ 上均存在候选，则认为该槽存在组内连续执行的可能，预留该组在该槽的最大等待时长：

$$
B_{lt}^{\mathrm{fai}}
=
\sum_{g\in G_{lt}^{\mathrm{fai}}}
\max_{(p,q)\in \mathcal{A}^{\mathrm{fai}}_g}\mathrm{lead}_{pq}
$$

其中 $G_{lt}^{\mathrm{fai}}$ 表示在 $(l,t)$ 上首件与余量都具备候选的 FAI 组集合，$\mathcal{A}^{\mathrm{fai}}_g$ 表示组 $g$ 内的 FAI 弧集合。

该口径比全局固定 buffer 更精细：不会对完全无 FAI 候选的槽扣减产能；同时又不引入新的二元变量，避免把主模型扩展为订单级时间模型。若业务担心多组 FAI 同槽过度保守，可配置上限：
...
例如 $B^{\mathrm{fai,max}}$ 可取 1 至 3 分钟。
...
$$
\sum_{d:(d,l,t)\in K}\frac{x_{dlt}}{\mathrm{uph}_{dlt}} + \mathrm{setup}_{lt} + B_{lt}^{\mathrm{fai}}
\le
\mathrm{Avail}_{lt}
$$

其中 $B_{lt}^{\mathrm{fai}}$ 为预处理得到的分钟级 FAI 等待预留。它只在容量上留出执行余量，不强制 FAI 首件与余量的槽级先后；实际先后仍由班内细排与时间轴校核处理。若未启用该预留，则取 $B_{lt}^{\mathrm{fai}}=0$。
...
在 `lead_time` 为分钟级、且业务允许首件与余量在同一班次内连续执行时，本剪枝不作为默认推荐。原因是主 MILP 的时间粒度是槽位，直接剪枝会把分钟级等待放大为槽级间隔，导致可行域过度收缩。此时推荐仅保留 FAI 弧数据，交由班内细排的轻量时间轴处理。
...
该形式表达的是槽级先后：余量需求所在槽的开始时刻不得早于首件需求的槽级完工下界加等待时长。它适合 FAI 等待时间较长、必须跨槽或跨班控制的场景；对于样例数据中 `lead_time` 约为 1 分钟的场景，默认不建议启用该 Big-M 约束，以免将分钟级等待解释为班次级分隔。
...
- FIX 与继承锁定中的 `seq`；
- FAI 弧集合 $\mathcal{A}^{\mathrm{fai}}$ 及 $\mathrm{lead}_{pq}$；
...
4. 同一 FAI 组内，首件需求 $p$ 优先于余量需求 $q$；
5. `model`；
6. `lot`；
7. `mo`；
8. `aps_schedule_order_ruler` 给出的材料键序列；
9. 合并后 demand 标识。

若 FAI 先后与显式锁定 `seq` 冲突，则锁定 `seq` 优先，冲突进入解后校核清单。
...
- `sequence`：槽内顺序号；
- `schedule_start_time`：按轻量时间轴计算的生产开始时间；
- `schedule_end_time`：按轻量时间轴计算的生产结束时间；
- `same_model_run`：机型连续段信息；
- `changeover_count`：班内实际换线次数摘要；
- `fai_wait_minutes`：FAI 等待占位分钟数；
...
### 14.5 轻量时间轴与 FAI 分钟级占位

对每个线体–槽位 $(l,t)$，在排序结果确定后，从槽开始时刻 $\sigma_t$ 起构造轻量时间轴。令游标为 $C$，初始值为：
...
$$
\Delta_d=\frac{x_{dlt}}{\mathrm{uph}_{dlt}^{\mathrm{used}}}
$$

对当前任务，先记更新前游标为 $C^{\mathrm{in}}$。若 $d$ 是某条 FAI 弧 $(p,d)$ 的余量需求，且首件 $p$ 的完成时刻 $\mathrm{Finish}(p)$ 已知，则其开始时刻取：

$$
\mathrm{Start}(d)=\max\{C^{\mathrm{in}},\ \mathrm{Finish}(p)+\mathrm{lead}_{pd}\}
$$

否则：

$$
\mathrm{Start}(d)=C^{\mathrm{in}}
$$

并定义：

$$
\mathrm{Finish}(d)=\mathrm{Start}(d)+\Delta_d
$$
...
当 $\mathrm{Start}(d)>C^{\mathrm{in}}$ 时，中间差值为等待占位。若输出表要求完整时间块，可将其回写为 `time_block_type = 1` 的空闲块；若输出只需要生产任务行，则至少应将后续任务的 `schedule_start_time` 顺延。

若主模型已启用 $B_{lt}^{\mathrm{fai}}$，则该等待在容量侧通常已有预留；二阶段仍需按实际顺序计算等待是否真正发生。本规则不引入额外求解器，也不改变主 MILP 的产量分配。若存在：

$$
\mathrm{Finish}(d)>\epsilon_t
$$

则不应截断完工时间，而应输出班内时间越界风险。自动跨班搬移属于更高阶修复逻辑，不作为 `v7.6` 默认要求。
...
对于分钟级 `lead_time`，FAI 校核不要求主 MILP 牺牲槽级可行域，而要求二阶段输出时间满足上述不等式。若因锁定顺序、产能已满或同槽时间越界导致无法满足，则输出风险，不在本版默认执行自动搬移。
...
- **二阶段轻量时间轴 + 容量预留**：推荐默认；适合分钟级 `lead_time`，仅在容量约束中扣减 $B_{lt}^{\mathrm{fai}}$，不增加主 MILP 变量规模；
- **候选剪枝**：规模较小，但会把等待解释为槽级门槛，适合较粗粒度 FAI；
- **Big-M 显式约束**：语义更强，但规模更大，适合小时级/跨班级 FAI 先后。
...
7. 在不引入订单级连续时间变量的前提下，补充**班内轻量时间轴**，用于输出 `schedule_start_time` / `schedule_end_time` 并处理分钟级 FAI 等待；
8. 在主 MILP 容量侧增加可选的 $B_{lt}^{\mathrm{fai}}$ 预留，使分钟级 FAI 等待不必进入先后约束，也不至于完全被容量忽略。

按照本版口径，主模型追求的是：在业务允许的稀疏候选集上，优先满足交付与高优先级需求，同时兼顾排产数量、整单完成、线体偏好、换线效率与分钟级 FAI 容量预留；而 lot 连续、材料排序链、精确换线顺序、FAI 真实先后与输出表所需的开始/结束时间则由二阶段轻量时间轴和审计层补足。这一划分既保留了数学模型的可解性，也尽量贴近 BOX APS 当前输入表和业务规则的真实口径。
