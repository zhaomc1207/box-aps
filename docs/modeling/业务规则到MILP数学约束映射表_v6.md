...
1. 主模型仍以 `demand_id` 为排产对象，不直接做 demand 合并。
...
以 `aps_schedule_demand_base_input` 为主表，结合 `aps_schedule_master_pn_input` 补齐：
...
- `aps_schedule_demand_base_input`：主需求表，进入主模型
...
1. 把 `priority_group` 的“组内且、组间或”显式纳入优先级定义
2. 不再把 `aps_merge_priority_setting` 误写成订单级附加权重
3. 把 `FIX` 改成与当前样例一致的整单精确锁定
4. 把 `aps_adjust_schedule_input` 改成“继承锁定 + 外部占能”双重口径
5. 把 D+2 出货目标统一落到 `ship_day_two`
6. 把 FAI 等待时长统一写成配置字段 `lead_time`
7. 把换线时间的匹配顺序和实际业务说明对齐
8. 明确若没有独立预留数据，则取 ($res_{lt}=0$)
